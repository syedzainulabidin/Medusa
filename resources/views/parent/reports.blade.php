        ...
        <div class="table-responsive">
            <table class="table">
                <thead>
                    <tr>
                        <td colspan="7">
                            <form action="{{ route('reports.parent') }}" method="GET"
                                ...
                                @if (request('search'))
                                    <button type="button" class="btns btn-danger"
                                        onclick="window.location.href='{{ route('reports.parent') }}'">Clear</button>
                                @endif
                            </form>
                ...
                <tbody>
                    @forelse($reports as $index => $report)
                        <tr>
                            ...
                            <td>{{ $report->date }}</td>
                            <td>
                                <form method="POST" action="{{ route('parent.report.show', $report->id) }}">
                                    @csrf
                                    <button class="btns status-available" type="submit"> <span
                                            class="material-icons">visibility</span>
                                    </button>
                                </form>
                                <form method="POST" action="{{ route('parent.report.download', $report->id) }}">
                                    @csrf
